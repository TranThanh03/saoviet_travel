...
public enum ErrorCode {
    UNKNOWN_ERROR(1001, "Đã xảy ra lỗi không xác định!", HttpStatus.OK),
    ...
    STARTDATE_INVALID(1027, "Ngày khởi hành không được trước 3 ngày so với hiện tại!", HttpStatus.OK),
    ...
    PROMOTION_STARTDATE_INVALID(1033, "Ngày bắt đầu không được trước ngày hôm nay!", HttpStatus.OK),
    ...
    CHECKOUT_EXITED(1043, "Giao dịch đã tồn tại!", HttpStatus.OK),
    INTERNAL_SERVER_ERROR(1044, "Lỗi máy chủ nội bộ!", HttpStatus.INTERNAL_SERVER_ERROR),
    CHECKOUT_NOT_EXITED(1045, "Giao dịch không tồn tại!", HttpStatus.OK),
    ...
    REDIS_ERROR(1070, "Redis service error!", HttpStatus.OK),
    AMOUNT_INVALID(1071, "Số tiền thanh toán không hợp lệ!", HttpStatus.OK),
    PROMOTION_USED(1072, "Khuyến mãi đã được sử dụng!", HttpStatus.OK),
    BOOKING_CAN_RESERVE(1073, "Vui lòng xác nhận giữ chỗ trước!", HttpStatus.OK),
    PROMOTION_USAGE_LIMIT_EXCEEDED(1074, "Khuyến mãi đã hết lượt dùng!", HttpStatus.OK),
    INVALID_CHECKOUT_CODE(1075, "Mã giao dịch không hợp lệ. Vui lòng kiểm tra lại!", HttpStatus.OK),
    BOOKING_EXPIRED(1076, "Lịch đặt đã hết thời gian thanh toán!", HttpStatus.OK),
    UPDATE_QUANTITY_INVALID(1077, "Số lượng phải từ 0 trở lên!", HttpStatus.OK),
    OTP_LENGTH_INVALID(1078, "OTP phải gồm đúng 6 chữ số!", HttpStatus.OK),
    OTP_INVALID(1079, "Mã OTP không hợp lệ hoặc đã hết hạn!", HttpStatus.OK),
    UPDATE_PASSWORD_FAILED(1080, "Thay đổi mật khẩu thất bại!", HttpStatus.OK),
    RESET_TOKEN_INVALID(1081, "Mã đặt lại mật khẩu không hợp lệ hoặc đã hết hạn!", HttpStatus.OK),
    ;
