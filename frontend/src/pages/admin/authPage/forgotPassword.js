...
const ForgotPasswordPage = () => {
    ...
    const handleOtpSubmit = async (e) => {
        ...
        try {
            const response = await AdminApi.verifyOTP({'email': email, 'otp': otp})

            if (response?.code === 1208) {
    ...
    const handleResendOtp = async (e) => {
        ...
        try {
            const response = await AdminApi.resendOTP({'email': email});
    ...
    const handleNewPasswordSubmit = async (e) => {
        ...
        try {
            const response = await AdminApi.resetPassword({'email': email, 'resetToken': resetToken, 'newPassword': newPassword});

            if (response?.code === 1209) {
            ...
            <div className="card forgot-card p-4 shadow-lg">
                ...
                {step === 1 && (
                    <form onSubmit={handleEmailSubmit} className="animate-fade">
                        <label>Nhập địa chỉ email của quản trị viên</label>
                        <input
                            type="text"
